...
df<-df_raw %>% 
  ...
  reshape_msd("long") %>%
  group_by(indicator, period, fundingagency, primepartner, period_type) %>%
...
tbl<-df3 %>%
  select(`primepartner`, `indicator`, `FY21 Q1`,`FY21 Q2`,`FY21 Q3`,`FY21 Total`,`FY21 Targets`, `FY21 Achieved`) %>%
  filter(`FY21 Q1`>0) %>%
  rename(Partner=primepartner) %>% 
 gt(groupname_col = "Partner",
